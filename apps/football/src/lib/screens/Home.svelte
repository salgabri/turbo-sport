<script lang="ts">
  ...
  // ── standings sorted (defensive: sort a copy by pts, then GD, then GF) ───
  ...
  // dot colour by table zone: top = accent, relegation (bottom 3) = down.
    ...
    <div class="rail">
          ...
          <div class="snap-body">
            ...
            {#each snapshotRows as row}
              <div class="snap-row" class:mine={row.mine}>
                ...
                <span class="c-a mono">{row.r.played}</span>
                <span
                  class="c-b mono"
      ...
      <section class="card">
        <div class="card-head solo">
          <span class="snap-title">Squad Status</span>
        </div>
        {#if hasSquad}
        ...
        {:else}
          <div class="empty">Squad status appears once players are on the books.</div>
        {/if}
      </section>
    </div>
  </div>
